...
function outputRagozás(igenév) {
	...
	szótő = igenév.slice(0, igenév.length - 3);
	végződés = igenév.slice(igenév.length - 3, igenév.length);
	if(rendhagyó && végződés == "ire") végződés = "isc";
	eredmény = "";
		for(var i = 0; i < 6; ++i) {
			igeMegRag = rendhagyók[igenév] === undefined?
				szótő + "<b>" + ragok[végződés][i] + "</b>"
				:
...
function isVowel(letter) {
	var mghk = ["a","e","i","o","u","á","é","í","ó","ú","ö","ő","ü","ű"];
	for(x in mghk) {
		if (letter == mghk[x]) {
			return true;
		}
